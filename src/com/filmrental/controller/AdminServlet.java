...
public class AdminServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("Username") != null) {
			...
			try {
				// recupero tutte le info dell'utente admin loggato e li passo
				// alla
				// jsp admin
				...
				// aggiunge un film alla collection dei film rentable o aggiunge un film suggerito alla collection dei film rentable
				if (request.getParameter("addfilm") != null) {
					if (request.getParameter("Add_Title") != "" && request.getParameter("Add_Regist") != ""
							&& request.getParameter("Add_Exit_Year") != "" && request.getParameter("Add_Quantity") != ""
				...
				if(request.getParameter("deletefilm")!= null){
					...
					if(ris==false){
						request.setAttribute("ErrorDelete", "Some copies of this film are rented. Return all copies from users before delete!");
					}
				}
