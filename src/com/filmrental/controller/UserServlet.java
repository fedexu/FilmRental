...
import com.filmrental.model.FilmRentVO;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (logout != null && logout.equals("logout")) {
			...
			if (session.getAttribute("Username") != null) {
				...
				try {
					// recupero tutte le info dell'utente loggato e li passo
					// alla
					// jsp
					...
					List<FilmRentVO> listfrentvo = new ArrayList();
					...
					// storico dei filmrented passando alla jsp un ValueObject
					// di FilmRented anzichè la list di entity.
					for (FilmRent filmr : listrented) {
						FilmRentVO fvo = new FilmRentVO(filmr.getRentId(), filmr.getFilm(), filmr.getOrderDate(), filmr.getReturnDate());
						listfrentvo.add(fvo);
					}
					...
					// disponibile
					BigDecimal[] number = f.getNumberOfFilmRentedCopies(list);

					// passo alla jsp i film disponibili e i film noleggiati,
					// aggiornati
					request.setAttribute("films", list);
					request.setAttribute("ListRented", listfrentvo);
